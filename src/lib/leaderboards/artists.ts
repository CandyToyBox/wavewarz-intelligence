...
/**
 * Shared artist-leaderboard computation — used by both the leaderboard page
 * and the public API, so the two can never drift (same lesson as
 * battle-metrics.ts: this used to be computed independently in two places).
 */
...
export type ArtistLeaderboardRow = {
  wallet: string
  name: string
  wins: number
  losses: number
...
export async function getArtistLeaderboard(): Promise<{ rows: ArtistLeaderboardRow[] }> {
  ...
  const clientRows: ArtistLeaderboardRow[] = rows.map(r => ({
    wallet: r.wallet,
    name: r.name,
    wins: r.wins,
    losses: r.losses,
