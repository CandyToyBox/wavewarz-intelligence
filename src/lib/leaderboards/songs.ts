...
/** Shared song/quick-battle leaderboard computation — used by both the leaderboard page and the public API. */
...
export async function getSongLeaderboard(): Promise<{ songs: SongData[] }> {
  const supabase = await createClient()
  // fetchAll paginates past the 1000-row cap — quick battles are nearing it.
  // NOTE: liveness is filtered in JS via isBattleLive() (timer math), not a SQL
  // status filter — `.neq('status','ACTIVE')` used to be here, but status text
  // is inconsistently cased across historical rows (ACTIVE/Active/ENDED/Ended),
  // which silently dropped ~78 genuinely-ended battles from the chart forever.
  ...
  const now = Date.now()
  const endedBattles = battles.filter(b => !isBattleLive(b, now))

  const map = new Map<string, SongData>()

  for (const b of endedBattles) {
